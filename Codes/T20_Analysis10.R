# Scorecard Link: http://www.espncricinfo.com/twenty20wc/engine/match/287869.html

mydata = read.csv("/home/kashish/Downloads/T20Matches/287869_req10.csv",row.names = NULL, header = F,skip = 21,colClasses = c("NULL",NA,NA,NA,NA,NA,NA,NA,NA,NA,NA))
...
balls = 1:length(pres)
par(mar = c(5,5,2,5))
plot(balls, pres, xlab= "", ylab="", axes= FALSE, col = ifelse(balls == wickets[1] | balls == wickets[2] | balls == wickets[3] | balls == wickets[4] | balls == wickets[5] | balls == wickets[6]  | balls == wickets[7] | balls == wickets[8] , "red", "black"), pch=ifelse(balls == wickets[1] | balls == wickets[2] | balls == wickets[3] | balls == wickets[4] | balls == wickets[5] | balls == wickets[6] | balls == wickets[7] | balls == wickets[8] , 19, 1), cex=ifelse(balls == wickets[1] | balls == wickets[2] | balls == wickets[3] | balls == wickets[4] | balls == wickets[5] | balls == wickets[6] | balls == wickets[7] | balls == wickets[8], 1.5, 1))
lines(balls,pres)
