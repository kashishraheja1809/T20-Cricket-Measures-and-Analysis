# Scorecard Link: http://www.espncricinfo.com/twenty20wc/engine/current/match/287853.html

mydata = read.csv("/home/kashish/Downloads/T20Matches/287853_req8.csv",row.names = NULL, header = F,skip = 20,colClasses = c("NULL",NA,NA,NA,NA,NA,NA,NA,NA,NA,NA))
...
balls = 1:length(pres)
par(mar = c(5,5,2,5))
plot(balls,pres,axes = FALSE,xlab = "", ylab = "", col= ifelse(balls == wickets[1] | balls == wickets[2] , "red", "black"), pch = ifelse(balls == wickets[1] | balls == wickets[2] , 19, 1), cex = ifelse(balls == wickets[1] | balls == wickets[2] ,2,1))
lines(balls,pres)
