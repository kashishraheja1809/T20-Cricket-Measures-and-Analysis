# Scorecard Link: http://www.espncricinfo.com/twenty20wc/engine/current/match/287866.html


mydata = read.csv("/home/kashish/Downloads/T20Matches/287866_req9.csv",row.names = NULL, header = F,skip = 21,colClasses = c("NULL",NA,NA,NA,NA,NA,NA,NA,NA,NA,NA))
...
runs = 1:length(balls)
par(mar= c(5,5,2,5))
plot(balls, pres, xlab= "", ylab="", axes = FALSE, col = ifelse(balls == wickets[1] , "red", "black"), pch=ifelse(balls == wickets[1], 19, 1), cex=ifelse(balls == wickets[1], 1.5, 1),ylim=c(0,max(pres)))
lines(balls,pres)
